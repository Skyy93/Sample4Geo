...
# Script for creating an pkl with the TOP_K furthest samples for each sample (train)
...
# G# Get furthest neighbors
values, ids = torch.topk(dm_torch, k=TOP_K, dim=1, largest=True)

# Convert to numpy for saving or further processing
values_far_numpy = values.numpy()
ids_far_numpy = ids.numpy()

far_neighbors = dict()
for train_id in train_ids:
    far_neighbors[train_id] = ids_far_numpy[train_id].tolist()

print("Saving...") 
with open("./data/gps_dict_256_furthest.pkl", "wb") as f:
    pickle.dump(far_neighbors, f)
