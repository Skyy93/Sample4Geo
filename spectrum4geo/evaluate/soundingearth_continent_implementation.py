...
def evaluate(config,
    ...
    continents = set(reference_dataloader.dataset.split_csv['continent']) 

    print("Compute Scores (world wide):")
    ...
    for continent in continents:
        print(f"Compute Scores for continent = {continent}")
        calculate_scores(query_features, reference_features, query_labels, reference_labels, step_size=step_size, ranks=ranks, continent=continent) 
        calculate_mean_distance(query_features, reference_features, query_labels, reference_labels, reference_chords, step_size=step_size, continent=continent)
        # calculate_roc_auc(query_features, reference_features, query_labels, reference_labels, step_size=step_size, continent=continent) -> to save time

    # cleanup and free memory on GPU
    if cleanup:
        del reference_features, reference_labels, reference_chords, query_features, query_labels, query_chords
        gc.collect()
...
def calculate_scores(query_features, reference_features, query_labels, reference_labels, step_size=1000, ranks=[1,5,10]):
    ...
    ranks_list = []
    topk.append(R//100) # <- thats the Top1 Recall
...
def calculate_fct_inside_continent(calculate_fct, query_features, reference_features, query_labels, reference_labels, step_size=1000, ranks=[1,5,10], eval_csv=None, continent="Europe"):
    ...
    continent_data = pd.read_csv(eval_csv)
    # Map from short_key to continent
    ref2continent = dict(zip(continent_data['short_key'], continent_data['continent']))
    
    # Filter reference features and labels for the specified continent
    ref_indices = [i for i, label in enumerate(reference_labels.cpu().numpy()) if ref2continent.get(label) == continent]
    filtered_reference_features = reference_features[:, ref_indices]
    filtered_reference_labels = reference_labels[ref_indices]

    # Filter query features and labels for the specified continent
    query_indices = [i for i, label in enumerate(query_labels.cpu().numpy()) if ref2continent.get(label) == continent]
    filtered_query_features = query_features[query_indices, :]
    filtered_query_labels = query_labels[query_indices]

    return calculate_fct(filtered_query_features, filtered_reference_features, filtered_query_labels, filtered_reference_labels, step_size=step_size, ranks=ranks)
...
def calculate_continent_recall(query_features, reference_features, query_labels, reference_labels, step_size=1000, ranks=[1,5,10], eval_csv=None):
    ...
    # Convert features and labels for easier manipulation
    query_labels_np = query_labels.cpu().numpy()
    reference_labels_np = reference_labels.cpu().numpy()
    
    # Get continent data for reference indices
    ref_continents = np.array([ref2continent.get(label, None) for label in reference_labels_np])

    # Initialize necessary variables
    topk = copy.deepcopy(ranks)
    topk.append(R // 100)  # Adjusted Top1 Recall to top 1% of the data
    Q = len(query_features)
    results = np.zeros([len(topk)])
    median_ranks = []
    
    # Calculate similarities in chunks due to step_size limitation
    similarity = []
    for i in range(0, Q, step_size):
        end = min(i + step_size, Q)
        sim_tmp = query_features[i:end] @ reference_features.T
        similarity.append(sim_tmp)
    similarity = torch.cat(similarity, dim=0)
    
    # Progress bar for displaying updates
    bar = tqdm(range(Q))
    
    for i in bar:
        query_continent = ref2continent.get(query_labels_np[i], None)
        scores = similarity[i]
        ...
        for rank, idx in enumerate(sorted_indices):
            if ref_continents[idx] == query_continent:
                if not correct_found:  # Only the first time the correct continent is found
                    median_ranks.append(rank + 1)
                    correct_found = True
                if rank < max(topk):
                    results[:np.searchsorted(topk, rank, side='right')] += 1
        
    bar.close()
    
    # Normalize results to get percentages
    results = (results / Q) * 100
    median_rank = np.median(median_ranks) if median_ranks else 0
    
    # Generate results string
    results_str = ' - '.join([f'Continent:- Recall@{k}: {results[idx]:.4f}' for idx, k in enumerate(topk[:-1])])
    results_str += f' - Continent:- Recall@top1: {results[-1]:.4f}'
    print(results_str)
    print(f'Continent:- Median Rank: {median_rank:.4f}')
    
    return results[0], median_rank


# noch nicht implementiert!
# = Korekte als Kontinent X vorhergesagt / alle als Kontinent X vorhergesagten
def calculate_continental_recall(query_features, reference_features, query_labels, reference_labels, step_size=1000, ranks=[1,5,10], eval_csv=None, continent="Europe"):
    # Load the CSV containing continent information
    continent_data = pd.read_csv(eval_csv)
    # Map from short_key to continent
    ref2continent = dict(zip(continent_data['short_key'], continent_data['continent']))
